...
// kafkaAuthFailurePatterns are output patterns indicating authentication
// failure from kafka-console-producer.
...
// checkIntegrationSkip skips the test if integration tests are not enabled.
// Set the INTEGRATION_TESTS environment variable to run integration tests.
func checkIntegrationSkip(t *testing.T) {
	t.Helper()
	if testing.Short() {
